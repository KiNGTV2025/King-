...
def fetch_data(url):
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=15) as response:
            ...
            # API yanıt formatı kontrolü
            if isinstance(data, list):
                return data
            elif isinstance(data, dict):
                if 'data' in data:  # Bazı API'ler data içinde liste döner
                    return data['data']
                elif 'movies' in data:  # Filmler için özel kontrol
                    return data['movies']
                elif 'series' in data:  # Diziler için özel kontrol
                    return data['series']
            return []
    except Exception as e:
...
def main():
    ...
    for category_id, category_name in film_kategorileri.items():
        for page in range(5):  # 5 sayfa ile sınırlı
            url = f"{base_url}/api/movie/by/filtres/{category_id}/created/{page}{SUFFIX}"
            data = fetch_data(url)
            stats['total_pages'] += 1
            
            # Film API'si farklı yanıt verebilir, debug log
            if page == 0:  # Sadece ilk sayfa için debug
                print(f"\nFilm Kategorisi: {category_name} (ID: {category_id})")
                print(f"API Yanıt Türü: {type(data)}")
                if isinstance(data, list) and len(data) > 0:
                    print(f"İlk kayıt: {data[0].get('title')}")
            
            for content in data if isinstance(data, list) else []:
    ...
    for entry in all_entries:
        try:
            ...
            print(f"JSON oluşturma hatası: {e}", file=sys.stderr)
            continue
